...
severity = 'Normal';
files = dir([drt '/Airflow/' severity]);

startindex = readmatrix(fullfile(drt,'Airflow',severity,'0Airflow_Indexes.csv'));
files = files(4:end);
J = length(files);
ratio = [0.05:0.05:0.2];
...
for j=1:J
    name = files(j).name;
    load(fullfile(drt,'Airflow',severity,name))
    fs = S.fs;
    T = 120;
    N = T*fs;
    sigma = 1e-6;
    ...
    save(fullfile(drt_r,severity,['Results_MissingDataAirFlow_' name '.mat']),'St')
end
