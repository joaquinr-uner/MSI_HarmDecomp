...
severity = 'Normal';
files = dir(fullfile(drt,'Plethysmogram',severity));

startindex = readmatrix(fullfile(drt,'Plethysmogram',severity,'0PPG_Indexes.csv'));
files = files(4:end);
J = length(files);
ratio = [0.05:0.05:0.2];
...
for j=1:J
    name = files(j).name;
    load(fullfile(drt,'Plethysmogram',severity,name))
    fs = S.fs;
    T = 60;
    N = T*fs;
    sigma = 1e-5;
    ...
    params_imp = {p_tlm,p_lse,p_dmd,p_gpr,p_arimaf,p_arimab,p_tbats,p_ddtfa,p_lsw,p_edmd,p_drago};
    ...
    St = struct('Err_Imp',Err_Imp,'Err_Spl',Err_Spl,'Err_Pch',Err_Pch,...
        ...
        'Times_Spl',Times_Spl,'Times_Pch',Times_Pch,'BestI',BestI,'BestS',BestS,'BestP',BestP);
    save(fullfile(drt_r,'Plethysmogram',severity,['Results_MissingDataPPG_' name '.mat']),'St')
end
