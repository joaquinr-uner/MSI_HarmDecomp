...
severity = 'Normal';
files = dir([drt '/NasalPressure/' severity]);

startindex = readmatrix(fullfile(drt,'NasalPressure',severity,'0NP_Indexes.csv'));
...
for j=1:J
    name = files(j).name;
    S = load(fullfile(drt,'/NasalPressure/',severity,name));
    fs = S.S.fs;
    T = 120;
    N = T*fs;
    sigma = 5e-7;
